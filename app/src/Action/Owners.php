...
class Owners extends AbstractAction
{
    ...
    private function createPrivacyTables($db) {
        ...
        CREATE TABLE $db.privacy_entry( uid VARCHAR(120) NOT NULL, created DATETIME DEFAULT CURRENT_TIMESTAMP, email VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, ref VARCHAR(100) DEFAULT NULL, surname VARCHAR(100) NOT NULL, form LONGTEXT DEFAULT NULL COMMENT '(DC2Type:json)', crypted_form LONGTEXT DEFAULT NULL, privacy LONGTEXT DEFAULT NULL, privacy_flags LONGTEXT DEFAULT NULL COMMENT '(DC2Type:json)', term_id VARCHAR(255) NOT NULL, domain VARCHAR(255) NOT NULL, site VARCHAR(255) NOT NULL, ip VARCHAR(100) DEFAULT NULL, telephone VARCHAR(120) DEFAULT NULL, deleted TINYINT(1) DEFAULT '0' NOT NULL, INDEX privacy_created (created), INDEX privacy_name_surname (name, surname), INDEX privacy_ref (ref), INDEX privacy_term_id (term_id), INDEX privacy_email (email), PRIMARY KEY(uid)) DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci ENGINE = InnoDB;
